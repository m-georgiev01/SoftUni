...
function solve() {
  ...
  function createTaskHandler() {
    ...
    const hasInvalidInputValue = Object.values(inputDOMSelectors).some(
      (input) => input.value === ''
    );
    ...
    const article = createElement(
      'article',
      null,
      otherDOMSelectors.tasksSection,
      ['task-card'],
      taskId
    );
    createElement(
      'div',
      `${label.value} ${labelToIconMap[label.value]}`,
      article,
      ['task-card-label', labelNameToClassMap[label.value]],
      null,
      true
    );
    createElement('h3', `${title.value}`, article, ['task-card-title']);
    createElement('p', `${description.value}`, article, [
      'task-card-description',
    ]);
    createElement('div', `Estimated at ${points.value} pts`, article, [
      'task-card-points',
    ]);
    createElement('div', `Assigned to: ${assignee.value}`, article, [
      'task-card-assignee',
    ]);
    const taskCardActions = createElement('div', '', article, [
      'task-card-actions',
    ]);
  ...
  function updateTotalPoints() {
    const totalSprintPoints = Object.values(tasks)
